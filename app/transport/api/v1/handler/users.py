...
@router.post("/users/{user_id}/devices/{device_id}", status_code=status.HTTP_201_CREATED, response_model=UserDevice)
...
@router.get("/users/{user_id}/analytics", status_code=status.HTTP_200_OK, response_model=UserAnalytics)
async def get_user_analytics(
    user_id: Annotated[uuid.UUID, Path(title="The ID of the user to retrieve analytics")],
    session: Annotated[AsyncSession, Depends(db_helper.get_session)],
